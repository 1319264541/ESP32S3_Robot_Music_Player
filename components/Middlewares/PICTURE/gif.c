...
/**
 * @brief       解码一帧（需先f_open + gif_getinfo + 设置lzw）
 * @param       file : 文件句柄（已打开，位置在movi后）
 * @param       gif  : GIF信息（lzw已初始化）
 * @param       x0,y0: 帧绘制偏移
 * @retval      0=成功, 1=错误, 2=EOF
 */
uint8_t gif_decode_one(FIL *file, gif89a *gif, uint16_t x0, uint16_t y0)
{
    return gif_drawimage(file, gif, x0, y0);
}

void gif_restore_ctbl(gif89a *gif) { gif_recovergctbl(gif); }
